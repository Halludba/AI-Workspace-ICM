...
31. **Persisted-state convergence guard.** Successful attempt completions are compared using an event-derived normalized fingerprint of durable run inputs, latest successful stage outputs/validation state, final artifacts, and next-stage cursor. Adjacent equality is stability, not a cycle. Re-entering an earlier non-adjacent persisted state after intervening change terminalizes the run as `CYCLE_DETECTED`. Hidden conversational or model state does not count as execution progress.
