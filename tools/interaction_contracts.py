...
def validate_policy(root:Path=ROOT)->dict:
    ...
    required={'schema_version','authority','grants_execution_authority','grants_mutation_authority','automatic_rule_deletion','components','invariants','contracts'}
...
def main()->int:
    ...
    sub.add_parser('validate'); show=sub.add_parser('show'); show.add_argument('contract_id'); ded=sub.add_parser('dedupe'); ded.add_argument('candidate')
    a=ap.parse_args()
    try:
        if a.cmd=='validate': out=validate_policy()
        elif a.cmd=='show': out=contract(a.contract_id)
        else: out=assess_dedup(_read(Path(a.candidate),'dedup candidate'))
