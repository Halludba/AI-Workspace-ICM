...
class PlanIntelligenceTests(unittest.TestCase):
 def task(self,**kw):
  x={'task_id':'T-a','objective':'Produce a bounded verified change','title':'Do bounded work','route_id':'tool-development','declared_scope':'SCOPED','target_role':'icm-runtime-architect','context_refs':['runtime'],'acceptance_criteria':['Done condition'],'verification':['Run targeted tests'],'status':'PENDING','depends_on':[]}; x.update(kw); return x
 def test_execution_task_sufficient(self): self.assertEqual(m.assess_task(self.task(),ROOT)['status'],'SUFFICIENT')
 def test_missing_objective_is_insufficient(self):
  x=self.task(); x.pop('objective'); r=m.assess_task(x,ROOT); self.assertEqual(r['status'],'INSUFFICIENT'); self.assertIn('objective',r['missing_fields'])
 ...
 def test_unknown_changed_task_fails_closed(self):
  p={'tasks':[self.task(task_id='T-a')]};
  with self.assertRaises(m.PlanIntelligenceError): m.reconcile(p,{'changed_task_ids':['T-missing'],'changed_context_refs':[]},ROOT)
 def test_workspace_cli_exposes_plan_surface(self):
  proc=subprocess.run([sys.executable,str(ROOT/'icm'),'plan','--help'],cwd=ROOT,text=True,capture_output=True); self.assertEqual(proc.returncode,0); self.assertIn('assess',proc.stdout); self.assertIn('reconcile',proc.stdout)
